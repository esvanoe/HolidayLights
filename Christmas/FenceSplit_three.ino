...
void loop() {
  ...
  colorChase(2, 5, strip.Color(0, 0, 255));    // reps, speed
  colorOff(10);    // wait

  colorChase(2, 5, strip.Color(255, 0, 0));    // reps, speed
  colorOff(10);    // wait
...
void colorChase(uint8_t reps, uint16_t speed, uint32_t color) {
  for(uint8_t count=0; count<reps; count++) {    
    for(uint16_t i=0; i<strip.numPixels()+350; i++) {
      ...
      if(i>50) {
        ...
        strip.setPixelColor(i-48, color);
        strip.setPixelColor(i-49, color);
        strip.setPixelColor(i-50, strip.Color(0, 0, 0));
      }
      if(i>100) {
        strip.setPixelColor(i-97, color);
        strip.setPixelColor(i-98, color);
        strip.setPixelColor(i-99, color);
        strip.setPixelColor(i-100, strip.Color(0, 0, 0));
      }
      if(i>150) {
        strip.setPixelColor(i-147, color);
        strip.setPixelColor(i-148, color);
        strip.setPixelColor(i-149, color);
        strip.setPixelColor(i-150, strip.Color(0, 0, 0));
      }
      if(i>200) {
        strip.setPixelColor(i-197, color);
        strip.setPixelColor(i-198, color);
        strip.setPixelColor(i-199, color);
        strip.setPixelColor(i-200, strip.Color(0, 0, 0));
      }
      if(i>250) {
        strip.setPixelColor(i-247, color);
        strip.setPixelColor(i-248, color);
        strip.setPixelColor(i-249, color);
        strip.setPixelColor(i-250, strip.Color(0, 0, 0));
      }
      if(i>300) {
        strip.setPixelColor(i-297, color);
        strip.setPixelColor(i-298, color);
        strip.setPixelColor(i-299, color);
        strip.setPixelColor(i-300, strip.Color(0, 0, 0));
      }
      if(i>350) {
        strip.setPixelColor(i-347, color);
        strip.setPixelColor(i-348, color);
        strip.setPixelColor(i-349, color);
        strip.setPixelColor(i-350, strip.Color(250, 250, 250));
      }
    strip.show();
    delay(speed);
    }
